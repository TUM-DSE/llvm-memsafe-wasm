...
public:
  ...
  bool SelectAddrOperands32Segment(SDValue Op, SDValue &Offset, SDValue &Addr);
...
private:
  ...
  bool SelectAddrOperands(MVT AddrType, MVT OffsetTy, unsigned ConstOpc, SDValue Op,
...
bool WebAssemblyDAGToDAGISel::SelectAddrOperands(MVT AddrType,
  ...
  if (N.getOpcode() == ISD::ADD &&
      SelectAddrAddOperands(OffsetTy, N, Offset, Addr))
    return true;
  ...
  if (N.getOpcode() == ISD::OR) {
    ...
    if (OrIsAdd && SelectAddrAddOperands(OffsetTy, N, Offset, Addr))
      return true;
  }

  // Fold constant addresses into the offset.
  if (ConstantSDNode *CN = dyn_cast<ConstantSDNode>(N)) {
    Offset = CurDAG->getTargetConstant(CN->getZExtValue(), DL, OffsetTy);
    Addr = SDValue(
        CurDAG->getMachineNode(ConstOpc, DL, AddrType,
                               CurDAG->getTargetConstant(0, DL, AddrType)),
        0);
  ...
  Offset = CurDAG->getTargetConstant(0, DL, OffsetTy);
  Addr = N;
  return true;
}
...
bool WebAssemblyDAGToDAGISel::SelectAddrOperands32(SDValue Op, SDValue &Offset,
                                                   SDValue &Addr) {
  return SelectAddrOperands(MVT::i32, MVT::i32, WebAssembly::CONST_I32, Op, Offset, Addr);
}

bool WebAssemblyDAGToDAGISel::SelectAddrOperands32Segment(SDValue Op, SDValue &Offset,
                                                   SDValue &Addr) {
  llvm_unreachable("Not implemented yet!");
  // return SelectAddrOperands(MVT::i64, MVT::i32, WebAssembly::CONST_I32, Op, Offset, Addr);
}

bool WebAssemblyDAGToDAGISel::SelectAddrOperands64(SDValue Op, SDValue &Offset,
                                                   SDValue &Addr) {
  return SelectAddrOperands(MVT::i64, MVT::i64, WebAssembly::CONST_I64, Op, Offset, Addr);
}
